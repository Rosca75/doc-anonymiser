...
| github.com/klippa-app/go-pdfium | v1.19.7 | in-place PDF text anonymisation (`backend/engine/exportfmt/pdfinplace.go`); Apache-2.0. Drives Google PDFium through a WebAssembly module run by the wazero runtime (pulled in transitively), so it is pure Go / CGo-free and honours the local-only, CGo-free guarantee (§4). The PDFium text-EDIT calls live behind the `pdfium_experimental` build tag: WITHOUT the tag the package still compiles but those calls fail at runtime, so PDF export silently falls back to the fpdf regeneration path. The shipped binary MUST therefore be built with `-tags pdfium_experimental`; CI and release wire this via the `PDFIUM_TAG` env, and `go test`/`go vet` run with it too. Local `wails dev` needs `-tags pdfium_experimental` to exercise true in-place editing. The pdfium WASM (~5 MB) is embedded by go-pdfium; no external asset ships |
| github.com/tetratelabs/wazero (via go-pdfium) | as required by go-pdfium v1.19.7 | pure-Go WebAssembly runtime that executes the pdfium.wasm module; Apache-2.0. Not imported directly; do not pin separately. This is the same wazero-WASM (P3) pattern §4 already permits as the PDF fallback |
