...
`detection_parity_test.go`, `preset_parity_test.go`, `image_parity_test.go`,
`value_shape_test.go`, `step_parity_test.go`,
...
- **The framework-agreement pair is the exception to "keep fixtures small", and
  it is deliberate.** `backend/testdata/framework_agreement.docx` and
  `framework_agreement_anon.docx` are a real eight-page contract and the same
  document as a human anonymiser produced it, at full size. They are the only
  fixture in the repository that exercises a real document's RUN FRAGMENTATION,
  its DEFINED-TERM vocabulary and its SIGNATURE BLOCKS together, and every one of
  those is load-bearing for a rule in the engine. Reducing them would remove the
  evidence, not the cost: the whole suite over them runs in well under a second,
  so they stay in the unit tier.

  Their ground truth is DATA, `backend/testdata/framework_agreement_expected.json`,
  and NEVER a golden markdown blob. A blob fails on every unrelated converter
  improvement and teaches the next session to regenerate it without reading it;
  the table fails only when a value stops being found, starts being found under
  the wrong category, or the review list floods again. The reproduction test
  compares against the reference with every `[LABEL_N]` normalised, and matches
...
- **Precision and recall are asserted with NUMBERS.** The review list a user
  ...
  numbers live in `TestFrameworkAgreementPrecision` and
  `TestFrameworkAgreementRecall`, with the reason for each bound beside it.
