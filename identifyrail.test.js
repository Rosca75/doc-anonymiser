...
test("every category checkbox is reachable without switching anything", () => {
  ...
  // Entity categories now appear in both Smart and Local AI sections, so we
  // deduplicate when checking (the test is "reachable", not "appears exactly once").
  const boxes = all(railHTML(), "input.cat-toggle").map((b) => b.attrs["data-category"]);
  const unique = Array.from(new Set(boxes)).sort();
  assert.deepEqual(unique, ALL_CATEGORIES.slice().sort());
});
