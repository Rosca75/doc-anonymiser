...
func TestEveryFrontendModuleWithLogicHasATest(t *testing.T) {
	...
	err := filepath.WalkDir("frontend", func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			...
			// neither holds application logic. wailsjs/ is GENERATED by `wails
			// build` and gitignored: it is not source, nobody may edit it, and a
			// test for it would be a test of the Wails code generator.
			//
			// Skipping it is not tidiness. Without it this guard passes on a clean
			// tree and fails the moment anyone runs `wails build` before
			// `go test ./...`, which is a failure that depends on the order the
			// gate was run in and says nothing about the code.
			if d.Name() == "docs" || d.Name() == "assets" || d.Name() == "wailsjs" {
				return fs.SkipDir
			}
			return nil
		}
