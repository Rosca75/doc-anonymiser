// tools/go.mod — the audit layer's tool dependencies, in a module of their own.
//
// WHY THIS IS NOT THE APPLICATION'S go.mod
//   `go get -tool` in the main module adds every tool's transitive dependency
//   graph to it: roughly 300 indirect requirements, and, worse, those
//   requirements take part in the main module's version resolution. Adding
//   golangci-lint there moved the application's own golang.org/x/text,
//   golang.org/x/sys and golang.org/x/net forward, which changes what
//   `wails build` compiles and what ships to users.
//
//   The audit layer must not be able to move the shipped binary. A separate
//   module makes that structural rather than a rule someone has to remember:
//   nothing here is in the application's build graph at all.
//   .github/workflows/audit.yml fails the job if building these tools has
//   touched the application's go.mod or go.sum.
//
// HOW THE TOOLS ARE RUN
//   Built into .audit/bin by `task tools`, and invoked from there. They are
//   NOT run with `go tool`, because `go -C tools tool deadcode ./...` would
//   resolve `./...` inside tools/ rather than against the application.
//
// VERSIONS
//   The four tool versions are recorded in CLAUDE.md §7 alongside the
//   application's pinned dependencies. Upgrading one changes what the audit
//   reports, so re-run the baseline (docs/audit-baseline.md) in the same
//   change.

module doc-anonymiser/tools
